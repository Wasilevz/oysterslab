...
const TYPE_COLORS: Record<ScheduleType, { bg: string; text: string; dot: string }> = {
  work: { bg: "bg-[var(--accent)]", text: "text-[var(--accent)]", dot: "bg-[var(--accent)]" },
  off: { bg: "bg-[var(--card-border)]", text: "text-[var(--muted)]", dot: "bg-[var(--muted)]" },
  vacation: { bg: "bg-amber-100", text: "text-amber-400", dot: "bg-amber-500" },
  sick: { bg: "bg-rose-100", text: "text-rose-400", dot: "bg-rose-500" },
};
    ...
    <div className="flex min-h-full flex-1 flex-col">
      <header className="border-b border-[var(--card-border)]/60 px-4 py-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="mt-1 text-2xl font-bold text-[var(--fg)]">{t("schedule.title")}</h1>
          </div>
        </div>
      </header>
      ...
      <div className="px-4 pt-4">
        <div className="rounded-2xl border border-[var(--card-border)] bg-[var(--card)] p-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-semibold text-[var(--fg)]">{t("schedule.myMonth")}</p>
      ...
      <div className="mt-3 flex-1 overflow-x-auto px-4 pb-24 relative">
        <table className="w-full border-collapse">
          <thead>
            <tr>
              <th className="sticky left-0 z-20 bg-[var(--card)] p-1.5 text-left text-[11px] font-medium text-[var(--muted)] min-w-[100px]">
                {t("salary.employee")}
              </th>
              {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day) => {
                ...
                return (
                  <th
                    key={day}
                    className={`p-1 text-center text-[11px] font-medium ${isWeekend ? "text-rose-400" : "text-[var(--muted-light)]"}`}
                  >
          ...
          <tbody>
            {employees.map((emp) => (
              <tr key={emp.id}>
                <td className="sticky left-0 z-20 bg-[var(--card)] py-1.5 pr-3 min-w-[100px]">
                  <p className="truncate text-xs font-semibold text-[var(--fg)]">{emp.full_name}</p>
                  {emp.position && (
                    <p className="truncate text-[9px] text-[var(--muted)]">{emp.position}</p>
                  )}
                </td>
                {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day) => {
      ...
      {workingToday.length > 0 && (
        <div className="border-t border-[var(--card-border)]/60 px-4 pt-4 pb-24">
