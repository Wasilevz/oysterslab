  ...
  return (
    <div className="px-4 pt-4 pb-8">
      <div className="mb-4 flex items-center gap-3">
        ...
        <h2 className="text-lg font-bold text-[var(--fg)]">{t("nav.shifts")}</h2>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex min-h-[40vh] items-center justify-center">
          <p className="text-sm font-medium text-[var(--muted)]">{t("shift.noOneOnShift")}</p>
        </div>
      ) : (
            ...
            return (
              <article
                ...
                className={`rounded-2xl border p-4 ${
                  isLongShift ? "border-amber-500/20 bg-amber-50" : "border-[var(--accent)]/10 bg-[var(--accent)]"
                }`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-lg font-bold text-[var(--fg)]">{user.full_name}</p>
                      {user.position && <p className="text-xs font-medium text-[var(--muted)]">{user.position}</p>}
                    </div>
                  </div>
                  <div className="text-right">
                    <ShiftTimer clockIn={shift.clock_in} className="font-mono text-2xl font-black tabular-nums text-[var(--accent)]" />
